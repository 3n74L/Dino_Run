...
> 드물게) 새로 계산합니다 - 캐시 적중 시에는 평범한 `drawImage` 한 번뿐이라 오버레이 방식보다
> 오히려 더 저렴합니다. `Dino.drawPart()`/`Obstacle.draw()`는 이제 `darknessAlpha`가 있으면
> `getDarkenedSprite()`가 반환한 이미지를, 없으면 원본 이미지를 그냥 그립니다.
>
