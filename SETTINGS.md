...
> `dino.update(deltaFactor)` 등에 전달하고, `apexGravityMultiplier`도 0.1→0.32로 재조정한 적이
> 있었는데, 사용자가 "애초에 baseSpeed=2를 기준으로 모든 오브젝트 속도를 배수로 조절하던 원래
> 방식"으로 즉시 롤백해달라고 요청해서 전부 되돌렸던 적이 있습니다.
>
> 이후 실제 서비스 배포 후, **개발 환경(240Hz 모니터)과 실제 사용자 기기(폰, 60~120Hz)의
> 주사율 차이 때문에 심각한 버그**가 발견됐습니다: `requestAnimationFrame`은 모니터 주사율만큼
> 자주 호출되는데 기존 코드는 "매 프레임 고정량 이동" 방식이라, 호출 빈도(주사율) 자체가
> 실제 체감 속도가 되어버렸습니다. 그 결과 저주사율 기기(대부분의 폰)에서 공룡 점프/배경
> 이동이 몇 배나 느려 보이고, 장애물 생성 간격(`setTimeout`, 실제 ms 기준이라 주사율 무관)은
> 그대로인데 이동 속도만 느려지니 **장애물끼리 겹쳐서 생성되는** 문제까지 있었습니다.
>
> **재도입 방식(1차 시도와의 차이점)**: 1차 시도 때는 물리 상수 자체를 재조정해서 체감이
> 달라졌던 게 문제였을 가능성이 커서, 이번엔 **기존 물리 상수(gravity, jumpStrength,
> riseGravityMultiplier 등)를 단 하나도 바꾸지 않고** `deltaFactor`(배율, 기본 1)만 각 이동/증분
> 계산에 곱하는 방식으로 최소 침습적으로 다시 넣었습니다. 기준 프레임을 **240fps**(개발
> 환경 = 사용자의 실제 테스트 모니터)로 잡아서, 240Hz에서는 `deltaFactor`가 항상 1에 가까워
> 기존 동작과 사실상 동일하고, 그보다 느린 기기에서만 부족한 이동량을 보정합니다. 즉
> **개발자 본인의 체감은 전혀 안 바뀌고, 폰 등 저주사율 기기의 체감만 개발 환경과 맞춰집니다.**
>
> | 항목 | 값/위치 |
> |---|---|
> | 기준 프레임 시간 | `FRAME_REFERENCE_MS = 1000/240` (`main.js`) |
> | 배율 상한 | `MAX_DELTA_FACTOR = 20` — 탭 전환 등으로 프레임이 한참 멈췄다 재개될 때 물리 폭주 방지 |
> | 배율 계산/리셋 | `computeDeltaFactor(timestamp)`, `resetFrameTiming()` (`main.js`) |
> | 리셋 호출 시점 | 일시정지 해제(`togglePause`), `restartGame()`, `reallyStartGame()` — 멈춰있던 시간이 다음 프레임에 몰아서 반영되지 않도록 |
> | 적용 대상 | `dino.update(deltaFactor)`, `background.update(deltaFactor)`, `obstacle.update(deltaFactor)`, `drawGameOverSequence(..., deltaFactor)`, `baseSpeed` 증가량, 점수(`currentScore`) 증가량 |
> | 적용 안 한 것(이미 실제시간 기준이라 불필요) | 점프 선입력(`jumpBufferTime`, `Date.now()` 기준), 장애물 생성 간격(`setTimeout`), 프테라노돈 날갯짓(`Date.now()` 기준), 밤/낮 전환 감속 구간 길이(`settleDurationMs`, `Date.now()` 기준), 공룡 다리 움직임(`Date.now()` 기준) |
> | 예외: RGB 색상 보간 | `colorLerpSpeed`는 지수 감쇠라 `deltaFactor`를 단순 곱하면 불안정해질 수 있어, `1 - (1-rate)^deltaFactor` 공식으로 정확히 일반화함(`background.js`) |
>
> ⚠️ **시행착오 기록(버그, 수정됨)**: `deltaFactor`를 `dino.js`의 `update()`에 한 번에 그대로
> 곱했더니(예: 모니터를 60Hz로 낮추면 `deltaFactor≈4`), **저주사율에서 점프 높이/시간이 오히려
> 훨씬 짧아져서 제일 작은 장애물도 못 넘는** 문제가 있었습니다. 원인은 상승/정점/하강 구간을
> 가르는 `apexVelocityThreshold`(=5, `vy`가 -5~+5 사이면 "정점"으로 판단해 중력을 거의 안
> 먹임)가 아주 좁은 창인데, `deltaFactor`가 크면 `vy`가 단 한 번의 `update()` 호출만에 이
> 좁은 창을 훌쩍 건너뛰어버려서(240Hz라면 여러 프레임에 걸쳐 조금씩 통과했을 구간을 60Hz는
> 한두 번의 큰 폭으로 관통) 정점에 머무는 실제 시간이 크게 줄어들었기 때문입니다. **해결**:
> `dino.js`의 `update(deltaFactor)`가 `deltaFactor`를 `Math.ceil(deltaFactor)`개의 작은
> 스텝(각각 "240fps 기준 한 프레임" 크기, `stepDelta = deltaFactor / steps`)으로 쪼개서
> `for` 루프로 반복 적용하도록 수정했습니다. 240Hz(`deltaFactor≈1`)에서는 `steps=1`이라
> 기존과 완전히 동일하고, 60Hz(`deltaFactor≈4`)에서는 `steps=4`로 나눠 240Hz와 같은 촘촘함
> 으로 물리를 계산하므로 정점 구간을 놓치지 않습니다. `background.js`/`obstacle.js`의 이동은
> 이런 임계값 분기가 없는 단순 선형 이동이라 여러 스텝으로 나누든 한 번에 계산하든 결과가
> 정확히 같으므로 여기는 손대지 않았습니다 — **이후 점프 물리에 새로운 임계값 기반 분기를
> 추가한다면 이 스텝 나누기 안에서 계산되도록 넣어야 이 문제가 재발하지 않습니다.**
>
> 이 영역(점프 물리/속도/장애물 생성 간격)을 다시 손볼 때는, **물리 상수 자체는 그대로 두고
> `deltaFactor` 곱셈 지점만 조정**하는 식으로 접근하세요 — 상수를 같이 바꾸면 1차 시도 때처럼
> 체감이 통째로 달라져서 원인 파악이 어려워집니다.
...
| 웨이브 생성 간격(기본 공식) | `max(900, max(700, 1100-speed*40)+packBonus / speed) + random(0~300)` (원래 공식) |
| 최소 안전 간격(신규, 바닥값으로 적용) | `getJumpCycleMs() + JUMP_SAFETY_BUFFER_MS(150)`, 공중 웨이브면 `* (speed / (speed - AIR_WORST_CASE_SLOWDOWN))`까지 곱해서 늘림 |
...
>
> 웨이브 생성 간격 공식 자체는 한때 "점프~착지 시간" 기준으로 여러 차례 조정했었지만,
> 최종적으로 사용자 요청에 따라 **원래의 `baseSpeed` 배수 기반 공식으로 완전히 롤백**한
> 적이 있습니다(그 공식 자체를 다시 만지지 말라는 뜻).

### 공중+지상 연속 배치 시 회피 불가능 버그 (신규, 수정됨)
지상 장애물 뒤에 낮게 나는 프테라노돈이 너무 가깝게 이어지면(또는 그 반대), 앞 장애물을
넘느라 아직 착지도 못 했는데 다음 장애물을 또 넘어야 하는 상황이 생겨 **물리적으로 회피가
불가능**한 경우가 있었습니다(공룡은 착지해야만 다시 점프 가능). 또한 서로 다른 두 번의
`spawnObstacle()` 호출(각자 다른 랜덤 재질)이 너무 가깝게 잡혀서 화면상 위치가 겹치는
문제도 있었습니다. 원인은 기존 간격 공식(위 "웨이브 생성 간격")이 순수 `baseSpeed` 기반
휴리스틱이라, "점프~착지에 실제로 얼마나 걸리는지"를 전혀 반영하지 않았기 때문입니다.

**해결(위 공식 자체는 그대로 두고, 그 위에 최소 안전 바닥값만 추가함)**: `main.js`의
`simulateJumpCycleMs()`가 `dino.js`의 실제 튜닝된 물리 상수(gravity, jumpStrength, 각 구간
중력 배율)를 240fps 기준으로 그대로 시뮬레이션해서 점프~착지에 걸리는 실제 시간(ms)을
계산합니다(`dino` 인스턴스 생성 후 처음 호출될 때 한 번만 계산 후 캐시). `nextSpawnTime`이
이 값 + `JUMP_SAFETY_BUFFER_MS`(150ms)보다 짧으면 그 값으로 올려 잡습니다. 이번 웨이브가
공중이면, 프테라노돈이 최악의 경우(`speedOffset` 최대 음수, 가장 느림)로 화면을 가로지를
때를 기준으로 필요 간격을 추가로 늘립니다(느릴수록 위협 구간에 더 오래 머무르므로).
`AIR_WORST_CASE_SLOWDOWN`(0.3)은 `obstacle.js`의 `speedOffset` 최대 편차와 반드시 같이
맞춰야 합니다 — 그쪽 편차 범위를 바꾸면 이 값도 같이 바꾸세요.

### 프테라노돈(공중 장애물) 속도 편차 (수정)
| 항목 | 값 |
|---|---|
| 편차 크기(`speedOffset`) | ±(0.1~0.3), 균등분포 (변경 없음) |
| 느림 허용 시점(신규) | `currentScore >= PTERA_SLOW_UNLOCK_SCORE`(= 250, `main.js`)일 때만 `speedOffset`이 음수(느림)가 될 수 있음. 그 전에는 항상 양수(빠름)로 고정 |

> ⚠️ **시행착오 기록(버그, 수정됨)**: `spawnObstacle()`의 다음 장애물 생성 타이머는
> `window.gameConfig.baseSpeed`(고정값)만 보고 계산하는데, 개별 프테라노돈은 여기에
> `speedOffset`(±0.1~0.3)이 랜덤으로 더해져 실제로는 그보다 빠르거나 느리게 움직입니다.
> 이 편차가 `baseSpeed`에서 차지하는 **상대적 비중은 baseSpeed가 작은 초반일수록 훨씬
> 큽니다**(예: baseSpeed=2일 때 최대 -15%, baseSpeed=12일 때 최대 -2.5%). 그래서 초반에
> "느린 프테라" 바로 뒤에 "빠른 프테라"가 이어 나오면(둘 사이 편차 합이 최대 0.6, 스폰
> 타이머는 이걸 전혀 모르고 계산) 실제 화면상 간격이 의도한 안전거리보다 훨씬 좁아져서
> 점프할 시간이 없이 죽는 경우가 있었습니다. 후반(baseSpeed가 오른 뒤)에는 같은 절대
> 편차라도 상대적 비중이 작아져서 문제가 되지 않았습니다.
>
> **해결**: 스폰 타이머 계산 자체(민감한 속도 아키텍처 영역)는 건드리지 않고, 대신
> `PTERA_SLOW_UNLOCK_SCORE`(250M) 이전에는 `speedOffset`의 부호를 항상 양수(빠름)로
> 고정해서 "느림" 자체가 나오지 않게 했습니다. 편차 크기(0.1~0.3)는 그대로 유지되어
> 다양성은 남아있고, 위험한 "느림+빠름" 조합이 초반 위험 구간에서만 사라집니다. 250M
> 이후엔 기존처럼 느림/빠름이 50:50으로 랜덤 결정됩니다.
...
| 어둠 계산 공식 (★공용 함수화됨) | `background.getDarknessAlpha(intensityMultiplier = 1.5)` — 검은 반투명 사각형의 알파값을 반환 |
| 장애물/공룡 전용 어둠 강도(신규) | `getDarknessAlpha(1.0)` — 배경(기본 1.5)보다 약하게 어두워짐 |

> ⚠️ **중요**: 어둠 정도는 이제 `background.getDarknessAlpha()` 한 곳에서만 계산합니다.
> `main.js`의 게임 루프와 `gameover.js`의 게임오버 연출 모두 이 함수를 호출해서 각자
> `draw(darknessAlpha)`에 넘겨야 합니다. 이 로직을 다시 손볼 때는 **절대 이 함수를 두 번
> 따로 만들지 마세요** (예전 "게임오버 시 필터 풀리는 버그"의 원인이 바로 이런 중복이었습니다).
> 달만 원본 색을 유지하고 배경만 어두워지는 것과 비슷하게, 장애물/공룡도 밤에 시야 확보를
> 위해 배경보다 덜 어둡게 하려고 `intensityMultiplier` 인자를 뒀습니다. `background.draw()`
> 내부(배경 레이어)는 인자 없이 기본값(1.5) 그대로 쓰고, `main.js`의 게임 루프와
> `gameover.js`는 `getDarknessAlpha(1.0)`으로 더 약하게 호출합니다.

> ⚠️ **시행착오 기록(버그, 수정됨)**: 해/달의 이동 경로가 곡선이어야 하는데(중간에서
> 고점을 찍고 다시 내려오는 포물선 모양, `getCelestialY(x)`가 sin 곡선으로 이미 구현돼
> 있었음) 실제로는 완전한 직선으로 보이는 문제가 있었습니다. 원인은 `getCelestialY()`가
> **정의만 되어 있고 `update()` 안에서 실제로 호출되는 곳이 없어서**, `celestialY`가
> 생성자의 초기값(`0`)에서 전혀 갱신되지 않고 있었기 때문입니다(해/달이 항상 캔버스 맨
> 위, y=0에서만 가로로 움직인 것). `update()`의 상태 관리 스위치문 직후에
> `this.celestialY = this.getCelestialY(this.celestialX);`를 추가해서 매 프레임
> `celestialX` 기준으로 곡선 높이를 다시 계산하도록 고쳤습니다.

> ⚠️ **시행착오 기록(성능 버그, 수정됨)**: 원래 어둠 표현은 `ctx.filter = 'brightness(...)'`
> 였는데, 게임이 진행될수록(특히 밤에) 프레임 드랍이 심해지는 문제가 있었습니다. 원인은
> 캔버스 `filter`가 켜진 상태에서 그려지는 `drawImage` **하나하나마다** 비용이 크다는 점 —
> 한 프레임에 배경 레이어 6번 + 장애물 N번 + 공룡 파츠 6번, 총 10~20개 가까운 `drawImage`가
> 전부 필터를 통과해야 했습니다. 게다가 `baseSpeed`가 오르면서 장애물이 팩(2~3개)으로 자주
> 나올수록 그려야 할 이미지 수 자체도 늘어나, "게임이 진행될수록 + 밤이면" 이중으로
> 심해졌습니다. **해결**: `brightness(b)` 필터는 각 채널에 `b`를 곱하는 것과 같고, 이는
> "검은색과 `(1-b)` 비율로 알파 블렌딩"하는 것과 수학적으로 동일합니다(검은색=0이므로
> `원본*(1-alpha) = 원본*b`가 되도록 `alpha = 1-b`). `getFilterString()` → `getDarknessAlpha()`
> 로 이름을 바꿔서 이 알파값을 반환하도록 했습니다.
>
> 배경 레이어는 화면 전체를 덮는 사각형이라 `background.draw()` 안에서 레이어를 그린 직후
> 반투명 검은 `fillRect`를 그냥 덧칠해도 문제없습니다(어차피 전체를 어둡게 하는 것이므로).
> `drawCelestial()`(달/해)은 그 뒤에 그려서 여전히 항상 원본 밝기를 유지합니다(순서상
> 레이어→어둠 덧칠→달/해 순으로 바뀌었지만, 화면에서 겹치는 영역이 없어 시각적으로는 차이
> 없음).
>
> ⚠️ **시행착오 기록(버그, 수정됨)**: 장애물/공룡에도 처음엔 똑같이 "그린 이미지 위에
> `fillRect` 덧칠" 방식을 썼는데, **이미지의 투명한 여백까지 네모나게 어두워져서 마치
> 히트박스 디버그 표시(사각형 테두리)가 켜진 것처럼 보이는** 문제가 있었습니다(공룡 팔다리는
> 회전 때문에 이미지 캔버스 여백이 넉넉해서 특히 눈에 띔). 캔버스가 알파(투명도)와 무관하게
> 사각형 영역 전체를 균일하게 어둡게 칠하기 때문입니다. **해결**: `background.js`에
> `getDarkenedSprite(img, darknessAlpha)`를 추가해서, 오프스크린 캔버스에 이미지를 그린 뒤
> `globalCompositeOperation = 'source-atop'`으로 어둠을 합성합니다 - 이 합성 모드는 원본
> 이미지의 알파(실루엣)를 그대로 존중하므로 투명한 여백은 계속 투명하게 남고 실루엣 모양
> 그대로만 어두워집니다. 결과물은 어둠 정도를 `DARKEN_BUCKET`(0.02) 단위로 반올림한 값을
> 키로 캐시해둬서, 매 프레임 다시 "굽지" 않고 어둠 단계가 실제로 바뀔 때만(하루 주기 동안
> 드물게) 새로 계산합니다(`DARKEN_BUCKET`, 이후 0.02→0.05로 완화됨) - 캐시 적중 시에는
> 평범한 `drawImage` 한 번뿐이라 오버레이 방식보다 오히려 더 저렴합니다.
> `Dino.drawPart()`/`Obstacle.draw()`는 이제 `darknessAlpha`가 있으면 `getDarkenedSprite()`가
> 반환한 이미지를, 없으면 원본 이미지를 그냥 그립니다.
>
> **앞으로 이 영역을 다시 손볼 때**: (1) `ctx.filter`를 다시 쓰지 마세요 - 성능 문제가
> 재발합니다. (2) 스프라이트(장애물/공룡처럼 투명 여백이 있는 이미지)를 어둡게 할 때는
> 절대 `fillRect`로 직접 덧칠하지 말고 반드시 `getDarkenedSprite()`를 거치세요 - 안 그러면
> 히트박스처럼 보이는 이 버그가 재발합니다. 화면 전체를 덮는 배경(레이어)처럼 투명 여백이
> 없는 경우에만 `fillRect` 직접 덧칠이 안전합니다.
>
> ⚠️ **시행착오 기록(버그, 수정됨)**: 실제 플레이 중 가끔(약 0.05초) 게임이 순간 멈췄다가
> 다시 진행되는 현상이 있었습니다. 원인은 어둠 버킷이 바뀌는 순간, **화면에 있는 모든
> 스프라이트(장애물 여러 개 + 공룡 파츠 6개)가 거의 같은 프레임에 동시에 새 버킷을 만나서
> 한꺼번에 캐시 미스가 나며 전부 새로 "구워졌기"** 때문입니다. **해결**:
> `warmDarkenedSpriteCache(images)`를 추가해서, 로딩이 끝나는 시점(`main.js`의 `checkLoad`
> 완료 핸들러)에 있을 수 있는 모든 어둠 버킷(`DARKEN_BUCKET` 간격으로 0~`MAX_DARKEN_ALPHA_
> TO_PREWARM`(0.5)까지)을 모든 이미지에 대해 미리 다 구워서 캐시를 채워둡니다. 이렇게 하면
> 실제 플레이 중에는 항상 캐시 적중만 일어나서 이 끊김이 사라집니다. **앞으로 어둠 처리
> 대상 이미지(새 장애물/공룡 파츠 종류 등)를 추가한다면, 이 프리웜 목록에도 포함되도록
> `main.js`의 `allSprites` 배열 구성을 확인하세요** — 안 그러면 그 이미지만 여전히 플레이
> 중 첫 버킷 전환 때 끊김이 발생합니다.

### 모바일/저사양 기기 추가 최적화 (신규)
램/성능이 부족한 모바일에서 배경 스크롤+밤 필터 구간에 "중간중간" 렉이 걸린다는 피드백으로
조사해서 3가지를 고쳤습니다(전부 시각적 결과는 동일, 순수 성능 개선):
...
> 캐시가 크게 불어날 상황이 아니라고 판단해 우선순위를 낮췄습니다. 나중에 장애물/공룡
> 스프라이트 종류가 크게 늘어나거나 캐시 관련 메모리 문제가 실제로 보고되면 그때 추가
> 검토하세요.
...
캔버스 DOM 엘리먼트에 걸리는 CSS `filter`라서, 캔버스 2D 컨텍스트 안에서 그리기 도중에만
적용되는 밤/낮 밝기 처리(`background.getDarknessAlpha()`)와는 완전히 별개이며 서로
간섭하지 않습니다.

### 일시정지 해제 시 2초 유예(신규, 현실 시간 패널티)
일시정지를 "화면을 멈춰놓고 장애물 배치를 여유롭게 관찰한 뒤 바로 반응하기"로 악용하지
못하게, "계속하기"를 눌러도 **오버레이(어두운 배경 + 계속하기/홈/재시작/소리 아이콘)가
2초에 걸쳐 서서히 사라지고, 그게 완전히 다 사라진 뒤에야 게임이 실제로 다시 움직이기
시작**합니다. 이 2초 동안은 `isPaused`가 계속 `true`라서 장애물도 안 움직이고 충돌도
없음 — 완전히 멈춰있는 상태 그대로입니다. 그래서 "계속하기 버튼을 누른 시점"과 "실제로
게임이 재개되는 시점" 사이에 강제로 2초의 현실 시간이 소모되는 구조입니다.

> ⚠️ **시행착오 기록**: 처음엔 반대로 구현했습니다 — 게임은 해제 즉시 정상 진행시키고
> 캔버스에 블러 필터를 걸어 2초에 걸쳐 맑아지게 하는 "시야 제한" 방식이었는데, 사용자가
> "이러면 무조건 죽을 수밖에 없다"고 정정했습니다. 장애물이 안 보이는데 이미 움직이고
> 있으면 회피가 불가능하기 때문입니다. 올바른 의도는 **오버레이가 다 사라지기 전까지는
> 게임 자체가 멈춰있어야** 하는 것이었습니다.

| 항목 | 값/위치 |
|---|---|
| 지속 시간 | `RESUME_TRANSITION_MS = 2000`(ms), `#pauseOverlay`의 `transition: opacity 2s ease;`와 반드시 같은 값으로 맞춰야 함 |
| 트리거 시점 | `togglePause()`의 일시정지 **해제** 요청 시(재시작/홈으로 가기 등 다른 경로는 해당 없음) |
| 정리 지점 | `goHome()`, `restartGame()`에서 `cancelResumeFade()` 호출 — 오버레이가 페이드 중일 때 홈으로 가거나 재시작하면, 남아있던 재개 타이머가 뒤늦게(홈 화면에 있는데) 게임 루프를 다시 켜버리는 버그를 방지 |

구현: `overlay.classList.add('fading-out')`로 CSS `opacity` 트랜지션(2초)을 시작시키고,
**동일한 2초를 `setTimeout`으로도 재서** 그 시점에야 `isPaused = false`로 바꾸고
`requestAnimationFrame(gameLoop)`로 실제 게임을 재개합니다. `isPaused`를 `togglePause()`
호출 즉시 바꾸지 않고 타이머 콜백 안에서 바꾸는 게 핵심입니다. 연타 방지를 위해
`resumeFadeTimeout`이 이미 있으면(페이드 진행 중) 재호출을 무시합니다.
...
| 트리거 | 파일 | 위치 |
|---|---|---|
| 플레이 버튼 클릭 (`notification.ogg`) | `main.js` | `startGameFromHome()` |
| 그 외 모든 `<button>` 클릭 (`button click.ogg`) | `audio.js` | `document`에 위임된 전역 클릭 리스너 (`#startBtn`만 제외) |
| 게임오버 (`Occupy.ogg`) | `gameover.js` | `gameOver()` |
| 최고 기록 경신 (`purchase.ogg`) | `gameover.js` | `gameOver()`의 `finalScore > bestScore` 분기(게임오버 효과음과 별개로 같이 재생됨) |
...
> 이 부분도 같이 수정해야 합니다.
>
> ⚠️ **시행착오 기록(버그, 수정됨)**: 처음 구현했을 때 배경음악 두 트랙(홈/인게임)이 전혀
> 들리지 않는 문제가 있었습니다. 페이지 로드 직후(사용자 제스처 전) `switchBgm()`이 곧바로
> `.play()`를 시도했었는데, 브라우저 자동재생 제한 때문에 이 첫 시도가 거부됐고, 이후
> 재시도가 제대로 안 붙었던 것으로 추정됩니다. `hasUserGesture` 플래그를 추가해서, 사용자가
> 페이지와 최소 한 번 상호작용하기 전에는 배경음악 쪽에서 **아예 `.play()`를 호출하지
> 않도록**(상태만 갱신) 바꾸고, 클릭/키/터치가 있을 때마다(한 번만이 아니라 매번)
> "재생돼야 하는데 멈춰있다면" 다시 재생을 시도하는 `ensureBgmPlaying()`으로 교체했습니다.
> 자기 자신을 계속 스스로 복구하는 방식이라 특정 브라우저의 정확한 자동재생 정책 차이에
> 덜 민감합니다.
>
> ⚠️ **공룡 점프 효과음은 제거됨**: `cloud.ogg`를 `Dino.jump()`에 연결했었는데, 점프를 워낙
> 자주 하다 보니 소리가 계속 반복돼서 귀에 거슬리고, 녹음 자체에 약간의 선딜레이가 있어서
> 실제 점프 타이밍과 0.1~0.2초 정도 어긋나 보인다는 피드백으로 완전히 제거했습니다
> (`sfx.jump`/`playJumpSfx()` 삭제, `dino.js`의 호출부도 제거).
>
> ⚠️ **시행착오 기록(버그, 수정됨)**: (점프 효과음이 있던 시절 발견됐던 버그, 참고용) 홈
> 화면에서도 점프 소리가 들리는 문제가 있었는데, 원인은 점프 효과음과 무관하게 원래 있던
> 잠재적 버그였습니다. `input.js`의 클릭/터치 리스너는 버튼/입력칸이 아닌 곳을 클릭하면
> 항상 `handleJump()`를 호출하는데, 에셋 로딩이 끝나면 `dino` 객체가 홈 화면에 있는 동안에도
> 이미 만들어져 있어서, 홈 화면에서 빈 곳을 클릭해도 `dino.jump()`가 조용히 호출되고
> 있었습니다(시각적으로는 문제없었음 - 그 dino
> 인스턴스는 실제 플레이 시작 시 새로 교체됨). 점프 효과음이 추가되면서 이게 들리는 버그로
> 드러났습니다. `handleJump()`에 `#gameScreen`이 실제로 보일 때만 동작하는 가드를 추가해서
> 해결했습니다.
>
> ⚠️ **볼륨**: 배경음악 2개(홈/인게임/아레나 공통 `BGM_VOLUME`)는 60%로 낮췄습니다. 버튼
> 클릭음을 native `<audio>.volume`의 상한(1.0)을 넘어 더 키워달라는 요청이 있어서 한때
> Web Audio API `GainNode`로 1.8배 증폭(`boostVolume()`)했었는데, `AudioContext`가
> `suspended` 상태에서 `resume()`이 비동기로 처리되는 타이밍 문제 때문에 실제 배포 사이트의
> 크롬에서 그 소리만 아예 안 들리는 문제가 있었습니다(로컬 파일/VS Code 프리뷰 등 환경마다
> 결과가 달랐음). 신뢰성이 더 중요하다고 판단해서 증폭은 포기하고 일반 `<audio>` 재생
> (`volume = 1`, 이미 상한)으로 되돌렸습니다. **"더 크게"는 native `<audio>.volume`(최대 1)
> 로는 원천적으로 불가능하고, Web Audio 라우팅은 환경별로 신뢰성 문제가 있었다는 걸 기억할 것.**
>
> ⚠️ **아레나 전환은 판당 한 번만**: `arenaBgmTriggered` 플래그로 관리하며, `restartGame()`/
> `reallyStartGame()`에서 새 판마다 `false`로 리셋됩니다. 점수 임계값(`ARENA_BGM_SCORE_THRESHOLD
> = 20000`)을 바꾸고 싶으면 `main.js`의 이 상수만 수정하면 됩니다.
>
> ⚠️ **랭킹 미리보기(`#rankingPreview`)는 `<button>`이 아니라 `<div>`**라서 전역 버튼 클릭음
> 리스너가 안 잡습니다. `toggleRanking()` 안에서 직접 `playButtonClickSfx()`를 호출해서
> 다른 버튼들과 동일하게 클릭음이 나도록 처리했습니다. 앞으로 버튼이 아닌(div 등) 클릭
> 가능 요소를 추가할 때도 이 패턴(직접 호출)을 따라야 합니다.
