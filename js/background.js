...
// [수정] 0.02 -> 0.05로 더 성기게: 이 정도 알파 차이는 어차피 눈으로 구별이 안 되고,
// 값을 키울수록 이미지당 캐시 단계 수가 줄어(예: 장애물/공룡 최대 알파 0.4 기준 20단계 ->
// 8단계) 하루 주기 동안 오프스크린 캔버스를 "새로 굽는" 이벤트 자체가 더 드물어진다.
// 시각적 의도(배경은 더 어둡게, 장애물/공룡은 덜 어둡게)는 전혀 바뀌지 않는다.
...
class Background {
    ...
    update(deltaFactor = 1) {
        ...
        // [버그 수정] getCelestialY()가 정의만 되어 있고 실제로 호출하는 곳이 없어서,
        // celestialY가 생성자의 초기값(0)에서 전혀 갱신되지 않고 있었다. 그 결과 해/달이
        // 가로로만 움직이고 세로로는 전혀 안 움직여서(항상 캔버스 맨 위, y=0) 곡선이 아니라
        // 완전한 직선 경로로 보이는 버그가 있었다. 매 프레임 celestialX 기준으로 다시
        // 계산해줘야 sin 곡선을 따라 중간에서 고점을 찍고 내려오는 궤적이 실제로 적용된다.
